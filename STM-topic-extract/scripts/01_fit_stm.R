...
if (USE_CHUNKS) {
  ...
  if (.GRP %% 100 == 0) cat(sprintf("  Processing document %d/%d\n", .GRP, .N))
    segment_one(doc_id, rg_number, text, CHUNK_TOKS)
  }, 
  by = doc_id]
  cat("Chunks created:\n")
  ...
  # carry metadata to each chunk
  cat("Adding metadata to chunks...\n")
  chunks <- merge(chunks_list, unique(docs[, .(doc_id, gender, year, interviewee)]), by="doc_id", all.x =TRUE)
  stm_input <- chunks
  stm_input[, parent_id := doc_id]           # remember original doc
  stm_input[, doc_id := sprintf("%s#%03d", parent_id, chunk_id)]  # unique id per chunk
} else {
  stm_input <- docs[, .(doc_id, rg_number, text, gender, year, interviewee)]
